/**
 * netlify/functions/scan.js — Website Jobs Score v3
 *
 * SELF-CONTAINED. No external require().
 * Works within Netlify's DEFAULT 10s function limit.
 * PageSpeed timeout set to 8s — safely within budget.
 *
 ...
 *   CRM_WEBHOOK_URL      ← optional (lead forwarding)
 */
...
function normaliseUrl(raw) {
  ...
  if (!s) return { valid: false, error: 'URL cannot be empty.' };
  ...
  if (h === 'localhost' || h === '127.0.0.1' || /^10\.|^192\.168\.|^172\.(1[6-9]|2\d|3[01])\./.test(h)) {
    return { valid: false, error: 'URL must be a public website.' };
  }
...
[PSI] Performance score: 43
  ...
  if (data.error) {
    console.error('[PSI] API returned error:', data.error.code, data.error.message);
    return { ok: false, fallback: true, reason: 'API error ' + data.error.code + ': ' + data.error.message, performanceScore: null };
  }
...
async function fetchScreenshot(url) {
  ...
  var d;
  try { d = await result.res.json(); } catch (e) { return { ok: false, fallback: true, screenshotUrl: null, title: null }; }
  if (d.status !== 'success') {
    console.warn('[Screenshot] Non-success status:', d.status);
    return { ok: false, fallback: true, screenshotUrl: null, title: null };
  }
...
function countSig(text, html, s, m, w) {
  var found = {}, weight = 0;
  (s || []).forEach(function(p){ if (text.indexOf(p) >= 0 || html.indexOf(p) >= 0){ found[p]='strong'; weight+=3; } });
  (m || []).forEach(function(p){ if (text.indexOf(p) >= 0 || html.indexOf(p) >= 0){ found[p]='medium'; weight+=2; } });
  (w || []).forEach(function(p){ if (text.indexOf(p) >= 0 || html.indexOf(p) >= 0){ found[p]='weak';   weight+=1; } });
...
async function fetchHomepage(url) {
  var result = await safeFetch(url, {
    headers: { 'User-Agent': 'Mozilla/5.0 (compatible; SiteScanner/1.0)', 'Accept': 'text/html', 'Accept-Language': 'en-GB' },
    redirect: 'follow',
  }, HOMEPAGE_TIMEOUT);

  if (!result.ok || !result.res) {
    console.warn('[Homepage] Fetch failed:', result.err);
    return { ok: false, error: result.err };
  }

  var ct = result.res.headers.get('content-type') || '';
  if (ct.indexOf('text/html') < 0 && ct.indexOf('xhtml') < 0) {
    console.warn('[Homepage] Non-HTML content-type:', ct);
...
function buildScore(psi, hp) {
  ...
  var ms = 10, mr = [];
  if (!psi.fallback && psi.performanceScore != null) {
    var mp = psi.performanceScore, fast = psi.mobileStrategy === 'FAST';
  ...
  } else mr.push('Mobile data unavailable');
  ...
  if (hp.ok) {
    ...
    var sf = Object.entries(hp.trust.found).filter(function(e){ return e[1]==='strong'; }).map(function(e){ return e[0]; });
  ...
  var total = clamp(Object.values(cats).reduce(function(a,c){ return a+c.score; }, 0), 0, 100);
  return { cats: cats, total: total };
}

function buildIssues(cats, hp) {
  var ranked = Object.entries(cats).sort(function(a,b){ return a[1].score - b[1].score; });
  var out = [], fixes = [];

  for (var i = 0; i < ranked.length && out.length < 5; i++) {
    var k = ranked[i][0], v = ranked[i][1];
  ...
  for (var j = 0; j < fc.length && fixes.length < 3; j++) {
    if ((cats[fc[j].k] && cats[fc[j].k].score || 0) <= fc[j].t) fixes.push(fc[j].f);
  }

  var fb = ['Ensure key info is visible without scrolling on mobile', 'Add a photo gallery of completed work', 'Include your address and Google Business Profile link in the footer'];
  for (var l = 0; l < fb.length && fixes.length < 3; l++) {
    if (fixes.indexOf(fb[l]) < 0) fixes.push(fb[l]);
  }
...
function buildMissed(total, trade, goal) {
  ...
  if (goal > 0) {
    var mid = Math.round(goal * adj), lo = Math.max(1, Math.round(mid*0.7)), hi = Math.round(mid*1.3);
    return { min:lo, max:hi, label: lo + '–' + hi + ' jobs per month', basis:'goal' };
  }
  var pct = Math.round(adj*100), plo = Math.max(pct-10,5), phi = Math.min(pct+10,90);
